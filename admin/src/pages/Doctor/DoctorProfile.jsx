  ...
  return profileData && (
    <div>
      <div className='flex flex-col gap-4 m-5'>
        ...
        <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white max-w-2xl shadow-sm'>
          {/* Doc Info */}
          <p className='flex items-center gap-2 text-3xl font-medium text-gray-700'>{profileData.name}</p>
          <div className='flex items-center gap-2 mt-1 text-gray-600'>
            <p>{profileData.degree} - {profileData.speciality}</p>
            <button className='py-0.5 px-2 border text-xs rounded-full font-medium'>{profileData.experience}</button>
          </div>
          ...
          <p className='text-gray-600 font-medium mt-4'>
            Appointment fee: <span className='text-gray-800'>{currency} {
              isEdit
                ? <input className='border rounded px-2 py-0.5 max-w-20' type="number" onChange={(e) => setProfileData(prev => ({ ...prev, fees: e.target.value }))} value={profileData.fees} />
                : profileData.fees
            }</span>
          </p>
          ...
          <div className='flex gap-1 pt-2 items-center'>
            ...
            />
            <label htmlFor="avail" className='cursor-pointer text-gray-600 text-sm font-medium'>Available</label>
          </div>
