      ...
      <div>
        {doctors.slice(0, 3).map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b border-gray-200' key={index}>
            <div>
              <img className='w-32 bg-indigo-50 rounded-lg' src={item.image} alt={item.name} />
            </div>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              ...
              <button className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded-md hover:bg-red-600 hover:text-white transition-all duration-300'>
                Cancel appointment
              </button>
            </div>
          </div>
