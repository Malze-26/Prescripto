          ...
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b border-gray-200' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && !item.isCompleted && (
                <button className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded-md hover:bg-primary hover:text-white transition-all duration-300 cursor-pointer'>
                  Pay Online
                </button>
              )}
              {!item.cancelled && !item.isCompleted && (
